...
// Start coding here
const totalUniqueMember = (bills) => {
  let result = bills.reduce((acc, bill) => {
    if (bill.member?.name && !acc.includes(bill.member?.name)) {
  ...
  return `Unique Members Count: ${result.length}`;
};

const totalMembers = totalUniqueMember(bills);
